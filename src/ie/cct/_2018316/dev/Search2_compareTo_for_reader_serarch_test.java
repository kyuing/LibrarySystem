...
public class Search2_compareTo_for_reader_serarch_test {
...
//		 key = key.trim().toUpperCase();
         ...
         while (right >= left) {
             ...
             if ( (key.compareToIgnoreCase(r.get(mid).getId()) == 0)
            		 || (key.compareToIgnoreCase(r.get(mid).getFname()) == 0)
            		 || (key.compareToIgnoreCase(r.get(mid).getLname()) == 0) ) {
